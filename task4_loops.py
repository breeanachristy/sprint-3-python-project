...
for sales in video_game_sales:
    if sales[GLOBAL_SALES]>25:
        print(sales[NAME],sales[GLOBAL_SALES])

pre_2000_count = 0
for year_released in video_game_sales:
    if year_released[YEAR]<2000:
        pre_2000_count = pre_2000_count + 1 
print(pre_2000_count)

total_NA_sales = 0
total_JP_sales = 0
for NA in video_game_sales:
    total_NA_sales = total_NA_sales + NA[NA_SALES]
for JP in video_game_sales:
    total_JP_sales = total_JP_sales + JP[JP_SALES]
...
if total_NA_sales > total_JP_sales: 
    print('The United States had higher sales')

nintendo_games = []
for games in video_game_sales:
    if games[PUBLISHER] == 'Nintendo':
        nintendo_games.append(games[NAME])
